...
import { Button, Container, Nav, Navbar, NavDropdown, Offcanvas, FormControl } from "react-bootstrap";
...
const Header = ({ logoLight = logoL, logoDark = logoD, showLogo = true, links = linkData, showSupport = false, showSearch = false, showNotification = false, showProfile = false, theme = "light", setTheme }) => {
    ...
    return (
        <Wrapper theme={theme}>
            <Navbar bg={theme === "dark" ? "dark" : "light"} className={`shadow-sm navbar-${theme === "dark" ? "dark" : "light"}`} expand="lg">
                <Container>
                    ...
                    {/* Logo */}
                    {showLogo && (
                        <Navbar.Brand href="/" className="mr-auto d-lg-block d-none" style={{ marginRight: "auto", marginLeft: 8, lineHeight: 0 }}>
                            <Image src={theme === "light" ? logoLight : logoDark} alt="Logo" height="30" width="auto" />
                        </Navbar.Brand>
                    )}
                    ...
                    <div className="d-flex align-items-center position-relative">
                        {showSearch && (
                            <>
                                {/* Search Bar */}
                                <form
                                    className={`search-bar-container ${showSearchBar ? "d-flex" : "d-none"} d-md-flex search-bar me-sm-3 me-2`} // Prevent form submission
                                >
                                    <Button variant="link" className={`${showSearchBar ? "d-flex" : "d-none"} d-md-none`} title="close" onClick={() => setShowSearchBar(false)}>
                                        <IoIosArrowBack color={theme === "dark" ? "white" : "black"} size={20} strokeWidth={0.5} />
                                    </Button>
                                    <FormControl ref={searchInputRef} type="text" placeholder="Search..." value={searchInput} aria-label="Search" style={{ width: "300px" }} onChange={(e) => setSearchInput(e.target.value)} />
                                    <Button variant="link" className="search-icon-btn m-0" title="Search">
                                        <CgClose onClick={handleRemoveText} color={theme === "dark" ? "#919ba6" : "#575a5e"} size={18} strokeWidth={0.5} />
                                    </Button>
                                </form>

                                {/* Search Toggle Button (Visible on small screens) */}
                                <Button variant="link" className={`search-icon-btn ${showSearchBar ? "d-none" : "d-flex"} d-md-none me-1`} title="Search toggle" onClick={handleToggleSearch}>
                                    <GoSearch color={theme === "dark" ? "#919ba6" : "#575a5e"} size={18} strokeWidth={0.5} />
                                </Button>
                            </>
                        )}
                        ...
                        {showNotification && (
                            <>
                                ...
                                >
                                    <GoBell color={theme === "dark" ? "#919ba6" : "#575a5e"} size={18} strokeWidth={0.5} />
                                </Button>
                                {showNotificationDropdown && (
                                    <div ref={notificationRef}>
                                        <NotificationsComponent theme={theme} showNotificationDropdown={showNotificationDropdown} setShowNotificationDropdown={setShowNotificationDropdown} />
                                    </div>
                                )}
                            </>
                        )}
                        {showSupport && (
                            <a href="/support">
                                <Button variant="link" className="p-0 me-sm-3 me-2" title="Support">
                                    <FaHeadset color={theme === "dark" ? "#919ba6" : "#575a5e"} size={18} />
                                </Button>
                            </a>
                        )}
